...
function _same_charge_positions(vortices::AbstractVector{PointVortex}, q::Integer, window::DiskWindow)
...
function ripley_k(vortices::AbstractVector{PointVortex}, r::Real, window::DiskWindow; charge::Integer = 1)
...
function besag_l(vortices::AbstractVector{PointVortex}, r::Real, window::DiskWindow; charge::Integer = 1)
...
function ripley_csr_baseline(
    ...
    mean_L ./= samples
    mean_H ./= samples
    return (; r = rs, K = mean_K, L = mean_L, H = mean_H, charge = Int(charge), window, samples)
end
...
function ripley_excess(
    ...
    n = count(vortex -> _vortex_charge(vortex) == q && _inside(window, xpos(vortex), ypos(vortex)), vortices)
    baseline = ripley_csr_baseline(n, r, window; charge = q, samples, rng)
    H = result.H .- baseline.H
    isempty(H) && return (; result, baseline, H, maximum = NaN, radius = NaN, integral = NaN)
    index = argmax(H)
    return (; result, baseline, H, maximum = H[index], radius = result.r[index], integral = _positive_integral(result.r, H))
end
