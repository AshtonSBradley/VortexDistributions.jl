...
struct CCMA
    ...
    function CCMA(; w_ma = 5, w_cc = 3, distrib = "hanning", distrib_ma = nothing, distrib_cc = nothing,
                  rho_ma = 0.95, rho_cc = 0.95)
...
function _get_weights(w, distrib, rho)
    weight_list = Vector{Vector{Float64}}()
...
function _get_alphas(points, curvatures)
    ...
    for idx in 2:(size(points, 1) - 1)
        ...
        if curvature != 0.0
            ...
            a_sin_arg = min(1.0, (dist_neighbors / 2) / radius)
            alphas[idx] = asin(a_sin_arg)
        else
            alphas[idx] = 0.0
        end
    end
...
function _get_descending_width(ccma::CCMA)
    descending_width_list = Vector{Dict{String, Int}}()
    w_ma_cur = ccma.w_ma
    w_cc_cur = ccma.w_cc
...
function _filter(ccma::CCMA, points, w_ma, w_cc, cc_mode)
    ...
    curvatures = vec(mapslices(norm, curvature_vectors, dims = 2))
...
function filter(ccma::CCMA, points::Matrix{Float64}; mode = "padding", cc_mode = true)
    ...
    if mode != "fill_boundary"
        result = _filter(ccma, points, ccma.w_ma, ccma.w_cc, cc_mode)
        return is_2d ? result[:, 1:2] : result
    end
