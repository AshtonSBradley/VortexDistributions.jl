...
function random_disk_vortices(rng::AbstractRNG, n::Integer, q::Integer, radius::Real)
    return [PointVortex(random_disk_point(rng, radius)..., q) for _ in 1:n]
end
...
function points_path(x, y, xmin, xmax, ymin, ymax, width, height, margin)
    ...
    return join(("$(round(xs[i], digits = 2)),$(round(ys[i], digits = 2))" for i in eachindex(xs)), " ")
end
...
function write_svg(path, r, homogeneous, clustered; calibrated_peak_scale)
    ...
    open(path, "w") do io
        ...
        println(io, """<text x="24" y="34" font-family="Helvetica,Arial,sans-serif" font-size="20" fill="#222">Same-sign Ripley excess in a disk</text>""")
        ...
        println(io, """<line x1="$margin" y1="$zero_y" x2="$(width - margin)" y2="$zero_y" stroke="#999" stroke-dasharray="5 5"/>""")
        for tick in xticks
            x = xcoord(tick, xmin, xmax, width, margin)
            println(io, """<line x1="$x" y1="$(height - margin)" x2="$x" y2="$(height - margin + 6)" stroke="#333"/>""")
            println(io, """<text x="$x" y="$(height - margin + 24)" text-anchor="middle" font-family="Helvetica,Arial,sans-serif" font-size="12" fill="#222">$(round(tick, digits = 1))</text>""")
        end
        for tick in yticks
            y = ycoord(tick, ymin, ymax, height, margin)
            println(io, """<line x1="$(margin - 6)" y1="$y" x2="$margin" y2="$y" stroke="#333"/>""")
            println(io, """<text x="$(margin - 10)" y="$(y + 4)" text-anchor="end" font-family="Helvetica,Arial,sans-serif" font-size="12" fill="#222">$(round(tick, digits = 1))</text>""")
        end
        println(io, """<line x1="$scale_x" y1="$margin" x2="$scale_x" y2="$(height - margin)" stroke="#111" stroke-width="2.5" stroke-dasharray="6 4"/>""")
        println(io, """<circle cx="$scale_x" cy="$(height - margin)" r="4" fill="#111"/>""")
        println(io, """<polyline fill="none" stroke="#4b6cb7" stroke-width="3" points="$hom_path"/>""")
        println(io, """<polyline fill="none" stroke="#c43c39" stroke-width="3" points="$clu_path"/>""")
        println(io, """<text x="$(width - 240)" y="72" font-family="Helvetica,Arial,sans-serif" font-size="15" fill="#4b6cb7">homogeneous - CSR</text>""")
        println(io, """<text x="$(width - 240)" y="96" font-family="Helvetica,Arial,sans-serif" font-size="15" fill="#c43c39">clustered - CSR</text>""")
        println(io, """<text x="$scale_x" y="$(margin - 14)" text-anchor="middle" font-family="Helvetica,Arial,sans-serif" font-size="14" font-weight="bold" fill="#111">reference r = $(round(calibrated_peak_scale, digits = 3))</text>""")
        println(io, """<text x="$(width / 2 - 40)" y="$(height - 16)" font-family="Helvetica,Arial,sans-serif" font-size="15" fill="#222">r</text>""")
        println(io, """<text x="14" y="$(height / 2)" transform="rotate(-90 14,$(height / 2))" font-family="Helvetica,Arial,sans-serif" font-size="15" fill="#222">H_excess(r)</text>""")
        println(io, "</svg>")
    end
end
...
homogeneous_summary = ripley_excess(homogeneous, r, window; charge = 1, samples = 300, rng = MersenneTwister(1))
clustered_summary = ripley_excess(clustered, r, window; charge = 1, samples = 300, rng = MersenneTwister(1))
