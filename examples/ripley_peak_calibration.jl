...
function calibrate_peak(
    ...
    println("Ripley-excess peak calibration for two Gaussian same-sign clusters in a unit disk")
    println("Each cluster has coordinate width σ; table reports median and 16-84% range.")
    println()
    println("σ       median r_peak   16-84% r_peak       median r_peak/σ   median r_peak/(2σ)")

    for σ in widths
        peaks = Float64[]
        for _ in 1:realizations
            vortices = clustered_disk_vortices(rng, n, 1, radius, σ)
            summary = ripley_excess(vortices, r, window; charge = 1, samples = baseline_samples, rng)
